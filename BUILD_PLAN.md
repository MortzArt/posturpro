...
- [x] **T16 — B2B landing page (offices, quote form)** (tier: standard)
...
- [x] **T17 — Admin: manual order entry (phone/offline orders)** (tier: standard; PlanResearch → UI Design → Dev → ReviewFix → QA PASS, HIGH, 2026-08-03; owner-approved scope addition 2026-08-03, extends the Phase 1 spec)
  Admin creates orders on behalf of customers (phone/in-showroom sales): order form under /admin/orders — customer contact + shipping address, catalog product/variant picker with live stock + server-recalculated prices (same trust rules as checkout), quantity per line, shipping from store settings (admin-overridable), internal note. Creates a real order through the existing atomic order-creation path (stock reservation, order number sequence, history log; source marked as manual/phone). Payment: no MP flow — admin marks pending_payment (charge later / on delivery) or records offline payment as paid. Confirmation email optional (phone customers may lack email — resolve against the NOT NULL contact_email constraint). Order then lives in the normal list/detail pipeline. `blocked by: T12`
...
- [x] **T19 — Homepage rebuild from client content page + product condition grades** (tier: full-cycle; ran high — all 12 stages, SHIP 9/10 HIGH, 2026-08-27; owner-requested)
  Rebuild the homepage using the STRUCTURE and COPY of the client-provided layout mockups (`tasks/reference/client-homepage-en.html` / `client-homepage-es.html`) while KEEPING the site's existing design language — the mockup's own visual design is rejected. Brand palette from the official logos (`public/brand/logo.svg`, `icon.svg`: #094220 / #0f7f3c / #e7f7ed) applied site-wide; ALL primary CTAs #f95326. Full shell restyle (topbar, header, footer, WhatsApp float) in our design language. New sections: values+impact, process, B2B block (reuse T16 quote flow), social proof, savings calculator, trust+FAQ, CTA banner. Homepage catalog pulls real products. NEW: condition-grade system (A+/A/B) — DB migration, admin product-management assignment UI, storefront grade badges (optional per product). Illustrative figures ship as swappable translation placeholders with an owner checklist. Full spec: `tasks/reference/T19-brief.md`. `blocked by: T14`

- [x] **T20 — Storefront restyle: Factorial design language, Phase A homepage (owner approval gate)** (tier: standard; PlanResearch → UI Design → Dev → ReviewFix → QA PASS HIGH, 2026-08-27; owner APPROVED Phase A homepage 2026-08-27 → Phase B unlocked as T21)
  Restyle the storefront (NOT admin) copying the layout/style grammar of https://factorialhr.com/factorial-it — typography scale, spacing, nav, cards, buttons, section layouts — while keeping the brand palette (greens + #f95326 CTAs) and logo, and replacing mint surface tints with pure white. Content/copy/data wiring from T19 unchanged. Phase A = homepage + shared shell only, served locally for owner review; Phase B (whole storefront) runs ONLY after explicit owner approval. Style source: live investigation → `tasks/reference/factorial-style-analysis.md`. Full spec: `tasks/reference/T20-brief.md`. `blocked by: T19`

- [x] **T21 — Storefront restyle Phase B: Factorial grammar across the remaining storefront** (tier: standard; PlanResearch → Dev → ReviewFix → QA PASS HIGH, 2026-08-27; owner approved Phase A 2026-08-27)
  Roll the T20 Factorial design language (DM Sans scoped to storefront, pure-white ground, orange pill CTAs, `.factorial-card`/`.stat-card`/`.pill-outline` primitives, hairline dividers, gradient moments, white footer) across every remaining storefront surface: catalog (/sillas, category/brand/style pages), search/filters, product detail page, cart, checkout, /empresas B2B landing, static pages (About/Contact/policies/FAQ/Showroom), and 404/error pages. Admin firewall untouched (Inter, no theme-storefront). Brand palette (greens + #f95326 CTA) + logo + all content/copy/data wiring unchanged. Shared components restyled deliberately (product-card, section-header, QuoteForm) — no per-page forks. Style authority: `tasks/reference/factorial-style-analysis.md`, T20 spec (`tasks/ui-design.md` from T20), DESIGN.md T20 amendment. `blocked by: T20`
