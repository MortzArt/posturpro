...
export function CheckoutSkeleton({ title }: { title: string }) {
  return (
    <div data-testid="checkout-skeleton" aria-hidden>
      ...
      <h1 className="mb-6 text-2xl font-semibold tracking-[-0.04em] text-foreground md:text-3xl">
        {title}
      </h1>
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[2fr_1fr] lg:gap-10">
        <div className="flex flex-col gap-6">
          {[0, 1, 2].map((card) => (
            <div key={card} className="factorial-card flex flex-col gap-4 p-4 md:p-5">
