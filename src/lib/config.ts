 ...
 *
 * The email SECRETS (`EMAIL_API_KEY`, `EMAIL_FROM_ADDRESS`, `EMAIL_OWNER_ADDRESS`)
 * live in `.env.local` and are read ONLY through `getEmailEnv()` in
 * `src/lib/env.ts`. The constants below are the NON-secret tunables + copy chrome.
 *
 * HOW TO SWAP REAL VALUES
 * -----------------------
