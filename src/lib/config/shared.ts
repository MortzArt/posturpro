/**
 * Cross-domain store identity + currency/locale/shipping seed constants.
 *
 * A2 split (see `src/lib/config.ts` header): these values are genuinely
 * cross-domain — currency/locale feed money formatting and i18n, shipping seeds
 * feed store_settings, and the WhatsApp + Supabase/seed values are store-wide —
 * so they live here rather than in any single feature module. Content moved
 * VERBATIM from the former monolithic `config.ts`; secrets remain in
 * `.env.local` via `src/lib/env.ts`.
 *
 * MONEY CONVENTION
 * ----------------
