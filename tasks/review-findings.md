...
- **n-1**: `refund-modal.tsx:65-66` — whole-peso-only refund (`^\d+$` → `amountPesos * 100`). Cannot partial-refund a centavo amount (e.g. $0.50 of a $199.50 payment). Matches the dev-done "whole pesos, Phase 1" decision; confirm the product intent.
- **n-2**: `order-status-stepper.tsx` — inline `style={{ transitionTimingFunction: "var(--ease-out)" }}` duplicates the Tailwind `ease-out` class; drop one.
- **n-3**: Hover-arrow `translate-x-0.5` nudges on the dashboard/indicator cards have no `prefers-reduced-motion` gate. A 2px translate is negligible, but the strict RM bar wants transforms gated.
- **n-4**: `packing-slip.ts:8-11` — the module docstring contains a rambling half-retracted sentence ("passes pre-formatted strings is avoided … No: we accept …"). Cosmetic; clean up — it reads like a thinking-out-loud draft.
...
| AC-10 | Email failure ≠ rollback; UI shows success | **PARTIAL** | Advance/cancel thread `emailSent`; **refund does NOT** (M-1) |
...
| AC-24 | Customer list paginated, email/name search, order counts | **PARTIAL** | Renders + searches; **counts truncate at scale** (M-3) |
| AC-25 | Dashboard new-order indicator linking to filtered list | **PARTIAL** | Present; **link filter ≠ count definition** (M-4) |
...
**Unmet / partial ACs: AC-10 (refund path only — M-1), AC-24 (M-3), AC-25 (M-4).** All three map to the majors above; none is critical.
...
| 7 | Email failure mid-transition — commit, log, not rolled back | **PARTIAL** | Advance/cancel surface `emailSent`; **refund drops it** (M-1) |
...
Not a block — no critical defect, and the money movement is sound. But M-1 through M-4 are must-fix before ship: M-1/M-2 mislead the Owner on the two most sensitive operations (refunds and cancellations), M-3 produces wrong customer data at scale, M-4 makes the dashboard headline metric self-contradictory. Fix the four majors (all mechanical), wire the three refund-input a11y attributes (m-3), and this is a clean ship.
